...
export default function CertificatePage() {
  ...
  const slides =
    mode === 'certificates' ? certificateSlides : awardSlides;
  ...
  const { dragDelta, handlers } = useDragSwipe(
    (dir) => move(dir),
    48
  );
  ...
  const visibleSlides = visibleOffsets.map((offset) => {
    const idx =
      (activeIndex + offset + slideCount) % slideCount;
    ...
    <main className="screen certificate-screen title-simple-page">
      <section className="certificate-panel">
        ...
        <div className="mode-buttons">
          <button
            className={
              mode === 'certificates' ? 'active' : ''
            }
            onClick={() => {
              setMode('certificates');
              goTo(0);
              setSelectedSlide(certificateSlides[0]);
            }}
          >
            CERTIFICATES
          </button>
          ...
          <button
            ...
            onClick={() => {
              setMode('awards');
              goTo(0);
              setSelectedSlide(awardSlides[0]);
            }}
          >
            AWARDS
          </button>
        ...
        <section className="certificate-body">
          ...
          <div className="left-panel">
            <div
              ...
              {...enhancedHandlers}
              style={{
                '--certificate-drag-x': `${dragDelta}px`,
              }}
            >
